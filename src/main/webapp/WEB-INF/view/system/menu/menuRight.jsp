...
<script src="${car}/static/layui_ext/dist/dtree.js"></script>
<script type="text/javascript">
    ...
    layui.extend({
        ...
        table.on('tool(menuTable)', function (obj) {
            ...
            if (layEvent === 'del') { //删除
                //判断当前菜单有没有子节点
                $.post("${car}/menu/checkMenuHasChildren.action?id=" + data.id, function (obj) {
                    if (obj.code >= 0) {
                        layer.msg("当前菜单有子节点，请先删除子节点");
                    } else {
                        layer.confirm('真的删除【' + data.title + '】这个菜单么？', function (index) {
                            //向服务端发送删除指令
                            $.post("${car}/menu/deleteMenu.action", {id: data.id}, function (res) {
                                layer.msg(res.msg);
                                //刷新数据表格
                                tableIns.reload();
                                //刷新左边的数
